...
zf=[1,1,1]/3; %intial feed composition
...
[~,idx_opt]=min(N_total);
xD2_opt=x_d2_grid(idx_opt); % optimum heavy key impurity to operate at 
R_opt=R_min(idx_opt);

N_total

% PART B 
R_grid = R_opt:0.1:10*R_opt;
...
for j = 1:length(R_grid)
    ...
    % COST CALCULATIONS 
    Q_reb =((V-D)*lhv_tl)/3600;
    Q_cond =(V*lhv_bz)/3600;
