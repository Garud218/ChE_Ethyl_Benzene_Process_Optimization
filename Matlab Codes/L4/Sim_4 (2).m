...
clear all
close all
% antoine coefficients array 
A=[6.905 6.954 6.99];
B=[1211.03 1344.8 1453.43];
C=[220.79 219.48 215.31];

P=1.1*750.06;
% doing analysis considering binary composition at a time 
% For Benzene and Xylene 
x=linspace(0,1,100);
for i=1:length(x)
    func = @(T)(P-x(i)*10^(A(1)-B(1)/(C(1)+T))-(1-x(i))*10^(A(3)-B(3)/(C(3)+T)));
    T1(i)=fsolve(func,25);
    y(i)=x(i)*10^(A(1)-B(1)/(C(1)+T1(i)))/P;
end
func=@(alpha,x)(alpha.*x./(alpha.*x+(1-x))); %non-linear least square curve fit 
alpha(1)=lsqcurvefit(func,1,x,y);


% For Toluene and Xylene (alpha toluene:alpha xylene)
for i=1:length(x)
    func = @(T)(P-x(i)*10^(A(2)-B(2)/(C(2)+T))-(1-x(i))*10^(A(3)-B(3)/(C(3)+T)));
    T2(i)=fsolve(func,25);
    y(i)=x(i)*10^(A(2)-B(2)/(C(2)+T2(i)))/P;
end
func=@(alpha,x)(alpha.*x./(alpha.*x+(1-x)));%non-linear least square curve fit
alpha(2)=lsqcurvefit(func,1,x,y);
alpha(3)=1;
global a
a=alpha;
%% TASK 1 TO GET A FEASIBILITY LIMIT IN DISTILLATE RECTIFYING 
clc
 
close all
F=100;
zf=[1/3,1/3,1/3]; % feed composition
a=alpha;  
...
% TASK 2 TO GET RMIN OF HEAVY KEY IMPURITY TILL FEASIBILITY LIMIT
